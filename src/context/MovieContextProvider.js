...
const MovieContextProvider = ({ children }) => {
  ...
  const getMoviesAndCartoons = async () => {
    try {
      const [moviesResponse, cartoonsResponse] = await Promise.all([
        axios.get(`${API_MOVIES}${window.location.search}`),
        axios.get(`${API_CARTOONS}${window.location.search}`),
      ]);
  ...
  const deleteCartoon = async (id) => {
    try {
      ...
      // Update the cartoon list after deletion
      getMoviesAndCartoons();
    } catch (error) {
